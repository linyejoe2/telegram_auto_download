...
if "%INNO_SETUP_PATH%"=="" (
    ...
    if exist "installer_output\TelegramAutoDownload-Setup-v0.5.1.exe" (
...
if "%ERROR_OCCURRED%"=="0" (
    ...
    if exist "installer_output\TelegramAutoDownload-Setup-v0.5.1.exe" (
        echo - Installer: installer_output\TelegramAutoDownload-Setup-v0.5.1.exe
    )
    echo.
    echo File sizes:
    ...
    if exist "installer_output\TelegramAutoDownload-Setup-v0.5.1.exe" (
        for %%A in ("installer_output\TelegramAutoDownload-Setup-v0.5.1.exe") do echo   Installer: %%~zA bytes
    )
    echo.
