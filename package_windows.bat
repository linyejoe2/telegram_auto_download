...
if "%INNO_SETUP_PATH%"=="" (
    ...
    if exist "installer_output\TelegramAutoDownload-Setup-v1.0.7.exe" (
...
if "%ERROR_OCCURRED%"=="0" (
    ...
    if exist "installer_output\TelegramAutoDownload-Setup-v1.0.7.exe" (
        echo - Installer: installer_output\TelegramAutoDownload-Setup-v1.0.7.exe
    )
    echo.
    echo File sizes:
    ...
    if exist "installer_output\TelegramAutoDownload-Setup-v1.0.7.exe" (
        for %%A in ("installer_output\TelegramAutoDownload-Setup-v1.0.7.exe") do echo   Installer: %%~zA bytes
    )
    echo.
